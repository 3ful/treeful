...
### Make Data available to Shiny App
...
The raster files downloaded from Copernicus CDS can simply be mounted into the Docker container. 
...
![Histogram of BIO06 of tree occurences](bio6_distribution_trees4f.png)

#### BIO08

![Histogram of BIO08 of tree occurences](bio8_distribution_trees4f.png)

#### BIO09

![Histogram of BIO09 of tree occurences](bio9_distribution_trees4f.png)
...
This approach means the R process will simply launch the shiny app. For a more robust deployment, use the other options. 
...
This currently a fairly cumbersome process. One isse is that the docker container installs the R package with the shiny app during build. There, secrets need to be available and the database needs to spun up. 

### Running in Shinyproxy

We abandoned this option. The main reason was that shinyproxy spins up containers for each user. Since this app requires a fair amount of Ram
